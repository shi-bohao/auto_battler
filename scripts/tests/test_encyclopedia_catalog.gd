...
func _run_tests() -> void:
	...
	_expect_int(catalog.get_category_order().size(), 5, "Encyclopedia should expose five categories.")
	...
	_expect_int(catalog.get_entry_count("heroes"), 3, "Hero category should include the three first-version heroes.")
