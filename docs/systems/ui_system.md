...
- 单位上方显示单位名称、星级、生命/魔力条和状态徽标。
- 名称字号已放大，便于战斗中识别。
- 信息显示位置整体向下移动。
- 生命值进度条按队伍区分：己方绿色、敌方红色；魔力值使用蓝色进度条，均有边框。
- 详细数值不堆在单位头顶，避免画面拥挤。
- 状态徽标同时显示控制状态与持续状态：控制为 `冻/晕/禁/嘲/缓`，燃烧为 `燃`，剧毒为 `毒N`。

### 战斗飘字

入口：`res://scripts/unit_feedback.gd`

`UnitFeedback.play_floating_number()` 统一处理战斗飘字，直接伤害、暴击、燃烧、剧毒和治疗均使用更高字号、黑色描边和随机横向偏移，避免草地背景或多段伤害导致看不清。
...
商品稀有度由 `RarityRollService` 按当前波次和 `RunModifierManager.luck` 抽取；商店使用 `SHOP` 遭遇类型，不附加精英/Boss 遭遇加成。抽到目标稀有度后，从对应栏位池中选择目标稀有度或最接近的可用商品。
...
| 文件 | 职责 |
| --- | --- |
| `res://scripts/shop_manager.gd` | 生成 10 格商品、区分已解锁单位/新单位/遗物、遗物去重、稀有度抽取、定价 |
| `res://scripts/game/rarity_roll_service.gd` | 商店与奖励共用的稀有度/幸运抽取规则 |
...
奖励稀有度抽取规则由 `RarityRollService` 维护，受当前波次、遭遇类型和 `RunModifierManager.luck` 影响。UI 只负责展示已抽出的奖励项，不在面板层重新计算概率。
...
13. 控制状态与燃烧/剧毒徽标应能同时显示，剧毒层数变化应同步刷新。
14. 普通伤害、暴击、燃烧、剧毒和治疗飘字应清晰可读。
