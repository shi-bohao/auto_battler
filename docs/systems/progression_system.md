...
> 最后更新：2026-06-05。本文档记录战斗胜利后的奖励三选一、节点三选一、商人、训练、事件和宝箱流程，是后续维护和扩展局内推进系统的权威参考。
>
...
- 遗物奖励：来自 `RelicManager.get_available_relic_reward_options()`，已拥有遗物不会重复出现。

稀有度基础概率：

| 稀有度 | 基础概率 | 每波成长 | 属性奖励数值 |
|---|---:|---:|---:|
| COMMON | 70% | +0% | 5 |
| FINE | 20% | +5% | 10 |
| RARE | 9% | +2% | 15 |
...
3. 幸运值由 `RunModifierManager.luck` 保存，按 `raw * (1 + luck / 100)` 放大，每个稀有度单项最高 100%。
...
- 奖励池读取 `RosterManager.get_all_unit_pool()`，因此会自动使用 `UnitCatalog` 的全量单位资源和 `catalog_id` 排序。
- 英雄专属单位由 `RosterManager.is_unit_id_available_for_run()` 过滤；只会出现当前英雄专属单位和通用单位。
- 召唤物、敌人、未进入玩家单位池的内容不会作为单位奖励。
- 阵容已满时不提供单位奖励。
- 抽到的指定单位奖励通过 `RosterManager.add_unit_by_id(unit_id)` 添加；随机单位通过 `RosterManager.add_random_unit()` 添加。
