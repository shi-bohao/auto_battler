...
| 文档 | 用途 | 维护建议 |
| --- | --- | --- |
| `project_status.md` | 当前项目进度、系统范围、近期性能修复、目录结构和验证命令入口 | 每次完成系统级功能后更新 |
| `future_features.md` | 功能规划与实现记录；金币经济遗物/远程弹道/非圆形AoE 均已完成，保留为实现参考 | 后续新功能先在此文档记录设计，再拆分实现；完成后保留实现入口 |
...
| `maggot_enemy_design.md` | 蛆虫族敌人（巨型蛆虫/蛆虫聚合体）设计，含被动/主动技能、腐痕 debuff 和接入说明 | 新增敌人或调整蛆虫族行为时更新 |
...
| 文档 | 当前状态 |
| --- | --- |
| `shop_ui_design.md` | 商店栏位、购买状态、遗物详情规则和购买单位后的预览复用策略基本仍可参考；按钮样式以当前代码生成像素风 UI 为准 |
| `reward_ui_design.md` | 奖励三选一规则仍可参考；当前实现已加入悬停详情、单位技能详情和英雄强化复用 |
| `ui_and_stats_design.md` | 主 UI、单位详情、遗物栏、统计面板和死亡链路刷新原则的历史设计说明；当前主菜单和战斗 HUD 已改为统一代码生成像素风，英雄选择界面与菜单层级以 `project_status.md` 为准 |
